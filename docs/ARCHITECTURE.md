# 시스템 아키텍처

Stockelper Airflow 시스템의 전체 아키텍처와 설계 결정을 설명합니다.

## 📐 전체 구조

```
┌─────────────────────────────────────────────────────────────┐
│                     Stockelper Ecosystem                     │
├─────────────────────────────────────────────────────────────┤
│                                                               │
│  ┌──────────────┐    ┌──────────────┐    ┌──────────────┐  │
│  │   Airflow    │    │    Neo4j     │    │   MongoDB    │  │
│  │  (21003)     │◄──►│ (21004/5)    │◄──►│   Atlas      │  │
│  └──────────────┘    └──────────────┘    └──────────────┘  │
│         │                                         ▲          │
│         │                                         │          │
│         └─────────────────────────────────────────┘          │
│                    stockelper network                        │
└─────────────────────────────────────────────────────────────┘
```

## 🏗️ 디렉토리 구조

```
stockelper-airflow/
├── dags/                           # Airflow DAG 정의
│   ├── stock_report_crawler_dag.py # 리포트 크롤러
│   ├── competitor_crawler_dag.py   # 경쟁사 크롤러
│   └── log_cleanup_dag.py          # 로그 정리
│
├── modules/                        # 재사용 가능한 모듈
│   ├── common/                     # 공통 유틸리티
│   │   ├── __init__.py
│   │   └── logging_config.py      # 통합 로깅 설정
│   ├── report_crawler/             # 리포트 크롤러
│   │   ├── __init__.py
│   │   └── stock_report_crawler.py
│   └── company_crawler/            # 기업 크롤러
│       └── compete_company_crawler.py
│
├── config/                         # 설정 파일
│   └── airflow.cfg                 # Airflow 설정
│
├── scripts/                        # 유틸리티 스크립트
│   ├── deploy.sh                   # 배포 스크립트
│   ├── setup_network.sh            # 네트워크 설정
│   ├── cleanup_logs.sh             # 로그 정리
│   └── stop.sh                     # 서비스 중지
│
├── docs/                           # 문서
│   ├── QUICKSTART.md               # 빠른 시작
│   ├── ARCHITECTURE.md             # 이 문서
│   ├── DEVELOPMENT.md              # 개발 가이드
│   ├── TROUBLESHOOTING.md          # 문제 해결
│   ├── API_REFERENCE.md            # API 레퍼런스
│   ├── LOGGING_GUIDE.md            # 로깅 가이드
│   ├── ADMIN_USER_SETUP.md         # Admin 설정
│   ├── DOCKER_COMPOSE_CHANGES.md   # Docker 변경사항
│   └── LOG_MANAGEMENT.md           # 로그 관리
│
├── docker-compose.yml              # Docker Compose 설정
├── Dockerfile                      # Docker 이미지 정의
├── requirements.txt                # Python 의존성
├── .env.example                    # 환경 변수 템플릿
├── .gitignore                      # Git 제외 파일
├── LICENSE                         # 라이선스
└── README.md                       # 프로젝트 개요
```

## 🔄 데이터 흐름

### 1. Stock Report Crawler

```
┌─────────────┐
│  Scheduler  │ (매일 00:00 UTC)
└──────┬──────┘
       │
       ▼
┌─────────────────────────────────────────────┐
│  stock_report_crawler_dag                   │
├─────────────────────────────────────────────┤
│                                             │
│  1. check_mongodb_connection                │
│     └─► MongoDB 연결 테스트                 │
│                                             │
│  2. crawl_stock_report                      │
│     ├─► StockReportCrawler 초기화           │
│     ├─► Selenium WebDriver 설정             │
│     ├─► 금융 웹사이트 접속                  │
│     ├─► 리포트 데이터 추출                  │
│     └─► MongoDB에 저장 (upsert)             │
│                                             │
│  3. report_results                          │
│     └─► 크롤링 통계 로깅                    │
│                                             │
└─────────────────────────────────────────────┘
       │
       ▼
┌─────────────┐
│   MongoDB   │
│  (Atlas)    │
│             │
│ Collection: │
│ stock_reports│
└─────────────┘
```

### 2. Competitor Crawler

```
┌─────────────┐
│  Scheduler  │ (매일 00:00 UTC)
└──────┬──────┘
       │
       ▼
┌─────────────────────────────────────────────┐
│  competitor_crawler_dag                     │
├─────────────────────────────────────────────┤
│                                             │
│  crawl_competitor_companies                 │
│     ├─► FinanceDataReader로 종목 코드 조회  │
│     ├─► Wisereport API 호출                 │
│     ├─► 경쟁사 데이터 파싱                  │
│     └─► MongoDB에 저장 (upsert)             │
│                                             │
└─────────────────────────────────────────────┘
       │
       ▼
┌─────────────┐
│   MongoDB   │
│  (Atlas)    │
│             │
│ Collection: │
│ competitors │
└─────────────┘
```

### 3. Log Cleanup

```
┌─────────────┐
│  Scheduler  │ (매일 02:00 UTC)
└──────┬──────┘
       │
       ▼
┌─────────────────────────────────────────────┐
│  log_cleanup_dag                            │
├─────────────────────────────────────────────┤
│                                             │
│  1. get_log_statistics                      │
│     └─► 현재 로그 상태 확인                 │
│                                             │
│  2. cleanup_old_logs                        │
│     ├─► 7일 이상 된 로그 파일 삭제          │
│     └─► 빈 디렉토리 정리                    │
│                                             │
│  3. get_log_statistics_after_cleanup        │
│     └─► 정리 후 상태 확인                   │
│                                             │
└─────────────────────────────────────────────┘
```

## 🐳 Docker 아키텍처

### 컨테이너 구성

```yaml
services:
  airflow:
    image: apache/airflow:2.10.4-python3.12
    container_name: stockelper-airflow
    ports:
      - "21003:8080"
    networks:
      - stockelper
    volumes:
      - airflow_logs:/opt/airflow/logs
      - airflow_db:/opt/airflow
```

### 네트워크 구성

```
┌─────────────────────────────────────┐
│     stockelper (bridge network)     │
├─────────────────────────────────────┤
│                                     │
│  ┌──────────────────────────────┐  │
│  │  stockelper-airflow          │  │
│  │  IP: 172.18.0.x              │  │
│  └──────────────────────────────┘  │
│                                     │
│  ┌──────────────────────────────┐  │
│  │  stockelper-neo4j            │  │
│  │  IP: 172.18.0.y              │  │
│  └──────────────────────────────┘  │
│                                     │
└─────────────────────────────────────┘
```

**장점**:
- 서비스 간 컨테이너 이름으로 통신 가능
- 격리된 네트워크 환경
- 외부 접근 제어 용이

## 💾 데이터 저장소

### MongoDB Collections

#### 1. stock_reports

```javascript
{
  "_id": ObjectId("..."),
  "date": "2025-10-12",
  "company": "삼성전자",
  "code": "005930",
  "title": "2025년 실적 전망",
  "summary": "...",
  "url": "https://...",
  "crawled_at": "2025-10-12T20:30:15.123Z"
}
```

**인덱스**:
```javascript
db.stock_reports.createIndex(
  { date: 1, company: 1, code: 1 },
  { unique: true }
)
```

#### 2. competitors

```javascript
{
  "_id": "005930",  // 기업 코드
  "target_company": {
    "code": "005930",
    "name": "삼성전자",
    "market_value": "400000000000000"
  },
  "competitors": [
    {
      "code": "000660",
      "name": "SK하이닉스",
      "market_value": "80000000000000"
    }
  ],
  "last_crawled_at": "2025-10-12T20:30:15.123Z"
}
```

**인덱스**:
```javascript
db.competitors.createIndex({ "_id": 1 })  // 기본 키
```

### SQLite (Airflow Metadata)

```
/opt/airflow/airflow.db
```

**테이블**:
- `dag`: DAG 메타데이터
- `dag_run`: DAG 실행 기록
- `task_instance`: Task 실행 기록
- `log`: Airflow 로그
- `variable`: Airflow 변수
- `connection`: 연결 정보

## 🔐 보안 아키텍처

### 환경 변수 관리

```
.env (gitignored)
  ├─► MONGODB_URI (민감)
  ├─► AIRFLOW_SECRET_KEY (민감)
  ├─► AIRFLOW_ADMIN_PASSWORD (민감)
  └─► MONGO_DATABASE (일반)
```

### 네트워크 보안

```
외부 → 21003 (Airflow UI) → stockelper network
                              ├─► airflow container
                              └─► neo4j container
```

**보안 계층**:
1. **Docker 네트워크 격리**: 내부 통신만 허용
2. **포트 매핑**: 필요한 포트만 외부 노출
3. **환경 변수**: 민감 정보 분리
4. **Admin 인증**: Airflow UI 접근 제어

## 🔄 실행 흐름

### 시스템 시작

```
1. docker compose up -d
   ├─► 네트워크 생성/연결
   ├─► 볼륨 마운트
   └─► 컨테이너 시작

2. Container Startup
   ├─► airflow db init
   ├─► airflow users create
   ├─► airflow scheduler (백그라운드)
   └─► airflow webserver (포그라운드)

3. Scheduler 시작
   ├─► DAG 파일 스캔
   ├─► DAG 파싱
   └─► 스케줄 등록

4. Webserver 시작
   ├─► Flask 앱 초기화
   ├─► UI 렌더링
   └─► 포트 8080 리스닝
```

### DAG 실행

```
1. Scheduler
   ├─► 스케줄 체크
   ├─► DAG Run 생성
   └─► Task 큐에 추가

2. Executor (SequentialExecutor)
   ├─► Task 하나씩 실행
   └─► 완료 후 다음 Task

3. Task 실행
   ├─► 환경 설정
   ├─► Python 함수 호출
   ├─► 로그 기록
   └─► 결과 저장 (XCom)

4. 완료
   ├─► DAG Run 상태 업데이트
   └─► 다음 스케줄 대기
```

## 📊 모니터링 아키텍처

### 로깅 계층

```
Application Layer
  ├─► modules/common/logging_config.py
  │   └─► setup_logger()
  │
  ├─► DAG Functions
  │   └─► logger.info/error/warning
  │
  └─► Module Classes
      └─► logger.debug/info

Airflow Layer
  ├─► Task Logs
  │   └─► /opt/airflow/logs/dag_id/task_id/date/attempt.log
  │
  └─► Scheduler Logs
      └─► /opt/airflow/logs/scheduler/

Docker Layer
  └─► Container Logs
      └─► docker logs stockelper-airflow
```

### 메트릭 수집

```
Airflow UI
  ├─► DAG 실행 통계
  ├─► Task 성공/실패율
  └─► 실행 시간

XCom
  ├─► crawl_result
  ├─► deleted_files
  └─► total_size_mb

MongoDB
  ├─► 문서 수
  └─► 컬렉션 크기
```

## 🔧 확장성 설계

### 수평 확장 (미래)

```
현재: SequentialExecutor
  └─► 단일 프로세스, 순차 실행

미래: CeleryExecutor
  ├─► Worker 1
  ├─► Worker 2
  └─► Worker N
```

### 수직 확장

```yaml
# docker-compose.yml
services:
  airflow:
    deploy:
      resources:
        limits:
          cpus: '2.0'
          memory: 4G
        reservations:
          cpus: '1.0'
          memory: 2G
```

## 🎯 설계 원칙

### 1. 모듈화
- 각 크롤러는 독립적인 모듈
- 공통 기능은 `modules/common/`에 통합
- DAG는 모듈을 조합하여 구성

### 2. 재사용성
- 공통 로깅 설정
- 공통 MongoDB 연결 로직
- 재사용 가능한 유틸리티 함수

### 3. 유지보수성
- 명확한 디렉토리 구조
- 상세한 문서화
- 일관된 코딩 스타일

### 4. 확장성
- 새 DAG 추가 용이
- 새 크롤러 추가 용이
- Executor 변경 가능

### 5. 보안
- 환경 변수로 민감 정보 관리
- 네트워크 격리
- 최소 권한 원칙

## 📚 참고 자료

- [Airflow Architecture](https://airflow.apache.org/docs/apache-airflow/stable/concepts/overview.html)
- [Docker Networking](https://docs.docker.com/network/)
- [MongoDB Best Practices](https://www.mongodb.com/docs/manual/administration/production-notes/)

---

이 아키텍처는 확장 가능하고 유지보수가 용이하도록 설계되었습니다.
