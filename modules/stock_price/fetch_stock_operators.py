...
class FetchStockDataOperator(BaseOperator):
    ...
    def execute(self, context: Context) -> pd.DataFrame | None:
        ...
        try:
            krx_df = fdr.DataReader(f'KRX:{self.symbol}', start=DEFAULT_START_DATE, end=to_date)
            yh_df = fdr.DataReader(f'YAHOO:{self.symbol}.KS', start=DEFAULT_START_DATE, end=to_date)
            ...
            if 'Not Found for url' in str(e) or isinstance(e, KeyError):
                log.warning(f"Could not find Yahoo data for {self.symbol}. Using KRX data only. Error: {e}")
                try:
                    krx_df = fdr.DataReader(f'KRX:{self.symbol}', start=DEFAULT_START_DATE, end=to_date)
                    df = krx_df[['Open', 'High', 'Low', 'Close', 'Volume']].copy()
                    df['Adj Close'] = df['Close']
                except Exception as krx_e:
                    log.error(f"Failed to fetch even KRX data for {self.symbol}: {krx_e}")
                    return None
            else:
                log.error(f"An unexpected error occurred while fetching data for {self.symbol}: {e}")
                return None
        ...
        df = df.rename(columns={
            ...
            'Volume': 'volume', 'Adj Close': 'adj_close', 'Date': 'date'
        })
